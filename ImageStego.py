...
class Stegno:
    ...
    art ="IMG-STEGO"
    art2 = "Encrypt or Decrypt a message in image using steganography"
    output_image_size = 0

    def main(self,root):
        root.title('ImageStegO')
        ...
        title.config(bg="black",fg="white",font=('courier',33))
        title.grid(pady=10)

        b_encode = Button(f,text="Encode",bg="#99ccff",command= lambda :self.frame1_encode(f), padx=14)
        b_encode.config(font=('courier',14))
        b_decode = Button(f, text="Decode",bg="#99ccff",padx=14,command=lambda :self.frame1_decode(f))
    ...
    def frame1_decode(self,f):
        ...
        l1.grid()
        bws_button = Button(d_f2, text='Select', bg="#99ccff",command=lambda :self.frame2_decode(d_f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(d_f2, text='Cancel',bg="#99ccff", command=lambda : Stegno.home(self,d_f2))
    ...
    def frame1_encode(self,f):
        ...
        bws_button = Button(f2,text='Select',bg="#99ccff",command=lambda : self.frame2_encode(f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(f2, text='Cancel',bg="#99ccff", command=lambda : Stegno.home(self,f2))
        back_button.config(font=('courier',18))
        back_button.grid(pady=15)
        back_button.grid()
        f2.grid()
